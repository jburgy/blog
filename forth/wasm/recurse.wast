;; A port of jonesforth to WebAssembly which keeps the FORTH return stack on the
;; WebAssembly call stack.
;;
;; tabulate.wast runs one flat `$next` loop and pushes `ip` onto a return stack
;; in linear memory whenever it meets a DOCOL codeword.  Here `$docol` *is* the
;; interpreter loop and DOCOL recurses into it, so every colon word gets a real
;; WebAssembly frame holding its own `ip` and EXIT is a plain `return`.
;;
;; The linear-memory return stack does not disappear entirely: >R, R>, RDROP,
;; RSP@ and RSP! are part of the language and jonesforth.f builds CATCH/THROW
;; out of them.  It shrinks to a *shadow* stack holding only what FORTH code put
;; there, and each frame remembers the shadow depth it started at ($base):
;;
;;   EXIT           rsp >= base  =>  return (nothing of ours is left to pop)
;;                  rsp <  base  =>  pop ip and keep going in this frame
;;   after a call   rsp >  base  =>  return as well, so a THROW that lifted rsp
;;                                   past us keeps unwinding towards its CATCH
;;
;; That is enough to make CATCH/THROW behave: CATCH pushes a saved DSP and the
;; address inside EXCEPTION-MARKER, so its own EXIT finds rsp < base and resumes
;; at EXCEPTION-MARKER instead of returning, while THROW's `RSP!` lifts rsp above
;; the bases of every frame between it and the CATCH.
;;
;; What it is *not* enough for is the idiom where a word pops its own caller's
;; return address -- `R>` reaching one cell below what the caller pushed.  Those
;; addresses now live in WebAssembly frames, not in linear memory, so they are no
;; longer FORTH data.  PRINT-STACK-TRACE therefore prints nothing, and regexp.f's
;; XCALL and (NNODE), which are built entirely on that trick, do not run at all.
;; The price of the wasm call stack is that the return stack stops being a stack
;; you can read.
;;
;; In exchange, nesting is bounded by the host's call stack (~8000 frames under
;; node 24) rather than by the 2048 cells at 0x2000, and `27 FIB` runs at about
;; the same speed as tabulate.wast.
(module
    ...
    ;;   0x2000 - 0x3FFF: shadow return stack (only what >R and friends push)
    ...
    (global $sp      (mut i32) (i32.const 0x2000))  ;; data stack pointer (grows downward), shared by every frame
    (global $rsp     (mut i32) (i32.const 0x4000))  ;; shadow return stack pointer, only >R & friends push here
    ...
    ;; Run the body of the colon word whose code field address is $cfa.
    (func $docol (param $cfa i32)
        (local $ip i32)   ;; instruction pointer (%esi)
        (local $sp i32)   ;; data stack pointer (grows downward) (%esp), cached from the global
        (local $base i32) ;; shadow return stack depth on entry
        (local i32 i32 i32 i32)

        (local.set $ip (i32.add (local.get $cfa) (i32.const 4)))
        (local.set $sp (global.get $sp))
        (local.set $base (global.get $rsp))
        ...
        (loop $next
            ...
            (call $docol (local.get $cfa))
            (local.set $sp (global.get $sp))
            ;; the callee left the shadow stack shallower than we found it: a THROW
            ;; is unwinding towards its CATCH and we are not it.
            (if (i32.gt_u (global.get $rsp) (local.get $base)) (then (return)))
            (br $next))
            ...
            ;; exit
            (if (i32.ge_u (global.get $rsp) (local.get $base))
                (then
                    (global.set $sp (local.get $sp))
                    (return)
                )
            )
            ;; FORTH pushed a return address of its own (CATCH does): honour it here
            ;; rather than returning, exactly as jonesforth's EXIT would.
            ...
            (i32.store (global.get $rsp) (i32.load (local.get $sp)))
            (local.set $sp (i32.add (local.get $sp) (i32.const 4)))
            (br $next))

            ;; r>
            (i32.store (local.tee $sp (i32.sub (local.get $sp) (i32.const 4))) (i32.load (global.get $rsp)))
            (global.set $rsp (i32.add (global.get $rsp) (i32.const 4)))
            (br $next))

            ;; rsp@
            (i32.store (local.tee $sp (i32.sub (local.get $sp) (i32.const 4))) (global.get $rsp))
            (br $next))

            ;; rsp!
            (global.set $rsp (i32.load (local.get $sp)))
            (local.set $sp (i32.add (local.get $sp) (i32.const 4)))
            (br $next))
