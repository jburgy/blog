/*	https://github.com/jburgy/blog/blob/main/forth/jansforth.c with the FORTH
	return stack moved onto the C call stack.

	jansforth.c runs one flat `while (1) switch (memory[cfa])` loop and pushes
	`ip` onto a return stack in `memory` on every DOCOL.  Here `docol` *is* that
	loop and DOCOL recurses into it, so each colon word gets a real C frame
	holding its own `ip` and EXIT is a plain `return`.

	The return stack does not disappear: >R, R>, RDROP, RSP@ and RSP! are part
	of the language and jonesforth.f builds CATCH/THROW out of them.  It shrinks
	to a *shadow* stack holding only what FORTH code put there, and each frame
	remembers the shadow depth it started at (`base`):
	...
	That is the same rule as forth/wasm/recurse.wast, where it is enough to make
	CATCH/THROW work: CATCH pushes a saved DSP and the address inside
	...
	pushed.  Those addresses now live in C frames, not in `memory`, so they are
	no longer FORTH data.  PRINT-STACK-TRACE prints nothing.  regexp.f's XCALL
...
static void docol(int cfa) {
    register int sp = dsp;
    register int ip = cfa + 1;
    const int base = rsp;
    ...
    while (1) {
        switch (memory[cfa]) {
            ...
            case EXIT:
                ...
                /* FORTH pushed a return address of its own (CATCH does): honour
                   it here rather than returning, as jonesforth's EXIT would. */
                ip = memory[rsp++];
                break;
            case LIT:
                memory[--sp] = memory[ip++];
