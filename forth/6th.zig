...
const offset = @divExact(@offsetOf(Word, "code"), @sizeOf(Instr));
...
fn InterpAligned(comptime alignment: mem.Alignment) type {
    return struct {
        ...
        pub inline fn next(self: *Self, sp: [*]isize, rsp: [*][*]const Instr, ip: [*]const Instr, target: [*]const Instr) void {
            ...
            const code: *const Code = @ptrCast(tgt[0]);
            return @call(.always_tail, code, .{ self, sp, rsp, ip[1..], tgt });
        }
...
const exit = defword(&invert, Flag.ZERO, "EXIT", _exit, &.{});
...
const lit = defword(&exit, Flag.ZERO, "LIT", _lit, &.{});
...
const here = defword(&state, Flag.ZERO, "HERE", _here, &.{});
...
const rz = defword(&version, Flag.ZERO, "R0", _rz, &.{});
...
const tor = defword(&o_nonblock, Flag.ZERO, ">R", _tor, &.{});
...
const fromr = defword(&tor, Flag.ZERO, "R>", _fromr, &.{});
...
const rspfetch = defword(&fromr, Flag.ZERO, "RSP@", _rspfetch, &.{});
...
const rspstore = defword(&rspfetch, Flag.ZERO, "RSP!", _rspstore, &.{});
...
const rdrop = defword(&rspstore, Flag.ZERO, "RDROP", _rdrop, &.{});
...
const word_ = defword(&emit, Flag.ZERO, "WORD", _word, &.{});
...
const number = defword(&word_, Flag.ZERO, "NUMBER", _number, &.{});
...
const find_ = defword(&number, Flag.ZERO, "FIND", _find, &.{});
...
const create = defword(&tdfa, Flag.ZERO, "CREATE", _create, &.{});
...
const comma = defword(&create, Flag.ZERO, ",", _comma, &.{});
...
const rbrac = defword(&lbrac, Flag.ZERO, "]", _rbrac, &.{});
...
const tick = defword(&colon, Flag.ZERO, "'", _tick, &.{});
...
const branch = defword(&semicolon, Flag.ZERO, "BRANCH", _branch, &.{});
...
const zbranch = defword(&branch, Flag.ZERO, "0BRANCH", _zbranch, &.{});
...
const litstring = defword(&zbranch, Flag.ZERO, "LITSTRING", _litstring, &.{});
...
const interpret = defword(&tell, Flag.ZERO, "INTERPRET", _interpret, &.{});
const _quit: [6]Instr = .{
...
const quit = defword(&interpret, Flag.ZERO, "QUIT", docol_, &_quit);
...
const char = defword(&quit, Flag.ZERO, "CHAR", _char, &.{});
...
const execute = defword(&char, Flag.ZERO, "EXECUTE", _execute, &.{});
...
const syscall1 = defword(&syscall2, Flag.ZERO, "SYSCALL1", _syscall1, &.{});
