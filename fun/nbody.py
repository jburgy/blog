...
from scipy.linalg.blas import daxpy, dspr2, zhpmv, zhpr  # pyright: ignore[reportAttributeAccessIssue]
...
for _ in range(20_000):
    a.real = x
    a.imag.fill(-1.0)  # pyright: ignore[reportAttributeAccessIssue]
    ap.fill(0.0)
    for i in range(3):  # A += α z z*
        zhpr(n, alpha=-2.0, x=a[i], ap=ap[i], overwrite_ap=True)
    sum(ap.real, axis=0, out=xxT)  # pyright: ignore[reportAttributeAccessIssue]
    xxT += 6
    ...
    divide(ap.imag, xxT, where=where, out=ap.imag)  # pyright: ignore[reportAttributeAccessIssue]
    ...
    daxpy(
        a=dt, x=a.real, y=v
    )  # v += a dt  # noqa E501  # pyright: ignore[reportAttributeAccessIssue]
